...
export const thread_sessions = sqliteCore.sqliteTable('thread_sessions', {
  ...
  source: sqliteCore
    .text('source', { enum: ['kimaki', 'external_poll'] })
    .notNull()
    .default('kimaki'),
...
export const session_events = sqliteCore.sqliteTable(
  'session_events',
  {
    id: sqliteCore.integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }).notNull(),
    session_id: sqliteCore.text('session_id').notNull(),
    thread_id: sqliteCore
      .text('thread_id')
      .notNull()
      .references(() => thread_sessions.thread_id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  ...
  (table) => [
    sqliteCore
      .index('session_events_session_id_timestamp_event_index_id_idx')
      .on(table.session_id, table.timestamp, table.event_index, table.id),
    sqliteCore
      .index('session_events_thread_id_timestamp_event_index_id_idx')
      .on(table.thread_id, table.timestamp, table.event_index, table.id),
  ],
)
...
export const part_messages = sqliteCore.sqliteTable('part_messages', {
  ...
  thread_id: sqliteCore
    .text('thread_id')
    .notNull()
    .references(() => thread_sessions.thread_id, { onUpdate: 'cascade' }),
  created_at: datetime('created_at').default(orm.sql`CURRENT_TIMESTAMP`),
})
...
export const bot_api_keys = sqliteCore.sqliteTable('bot_api_keys', {
  app_id: sqliteCore
    ...
    .primaryKey()
    .notNull()
    .references(() => bot_tokens.app_id, { onUpdate: 'cascade' }),
...
export const thread_worktrees = sqliteCore.sqliteTable('thread_worktrees', {
  ...
  status: sqliteCore
    .text('status', { enum: ['pending', 'ready', 'error'] })
    .notNull()
    .default('pending'),
...
export const channel_models = sqliteCore.sqliteTable('channel_models', {
  channel_id: sqliteCore
    .text('channel_id')
    .primaryKey()
    .notNull()
    .references(() => channel_directories.channel_id, { onUpdate: 'cascade' }),
...
export const channel_verbosity = sqliteCore.sqliteTable('channel_verbosity', {
  channel_id: sqliteCore
    ...
    .notNull()
    .references(() => channel_directories.channel_id, { onUpdate: 'cascade' }),
  verbosity: sqliteCore
    .text('verbosity', { enum: ['tools_and_text', 'text_and_essential_tools', 'text_only'] })
    .notNull()
    .default('tools_and_text'),
  updated_at: datetime('updated_at')
    .default(orm.sql`CURRENT_TIMESTAMP`)
    .$onUpdate(() => new Date()),
})
...
export const scheduled_tasks = sqliteCore.sqliteTable(
  'scheduled_tasks',
  {
    id: sqliteCore.integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }).notNull(),
    status: sqliteCore
      .text('status', { enum: ['planned', 'running', 'completed', 'cancelled', 'failed'] })
      .notNull()
      .default('planned'),
    ...
    channel_id: sqliteCore
      .text('channel_id')
      .references(() => channel_directories.channel_id, {
        onDelete: 'set null',
        onUpdate: 'cascade',
      }),
    thread_id: sqliteCore
      .text('thread_id')
      .references(() => thread_sessions.thread_id, { onDelete: 'set null', onUpdate: 'cascade' }),
...
export const session_start_sources = sqliteCore.sqliteTable(
  'session_start_sources',
  {
    session_id: sqliteCore.text('session_id').primaryKey().notNull(),
    schedule_kind: sqliteCore.text('schedule_kind', { enum: ['at', 'cron'] }).notNull(),
    scheduled_task_id: sqliteCore
      .integer('scheduled_task_id', { mode: 'number' })
      .references(() => scheduled_tasks.id, { onDelete: 'set null', onUpdate: 'cascade' }),
    created_at: datetime('created_at').default(orm.sql`CURRENT_TIMESTAMP`),
    updated_at: datetime('updated_at')
      .default(orm.sql`CURRENT_TIMESTAMP`)
      .$onUpdate(() => new Date()),
...
export const forum_sync_configs = sqliteCore.sqliteTable(
  'forum_sync_configs',
  {
    id: sqliteCore.integer('id', { mode: 'number' }).primaryKey({ autoIncrement: true }).notNull(),
    app_id: sqliteCore
      .text('app_id')
      .notNull()
      .references(() => bot_tokens.app_id, { onDelete: 'cascade', onUpdate: 'cascade' }),
  ...
  (table) => [
    sqliteCore
      .uniqueIndex('forum_sync_configs_app_id_forum_channel_id_key')
      .on(table.app_id, table.forum_channel_id),
  ],
)

export const ipc_requests = sqliteCore.sqliteTable(
  'ipc_requests',
  {
    id: sqliteCore
      .text('id')
    ...
    status: sqliteCore
      .text('status', { enum: ['pending', 'processing', 'completed', 'cancelled'] })
      .notNull()
      .default('pending'),
...
export const relations = defineRelations(
  ...
  (r) => ({
    thread_sessions: {
      ...
      thread_worktree: r.one.thread_worktrees({
        from: r.thread_sessions.thread_id,
        to: r.thread_worktrees.thread_id,
      }),
      ipc_requests: r.many.ipc_requests(),
    },
    session_events: {
      thread: r.one.thread_sessions({
        from: r.session_events.thread_id,
        to: r.thread_sessions.thread_id,
      }),
    },
    part_messages: {
      thread: r.one.thread_sessions({
        from: r.part_messages.thread_id,
        to: r.thread_sessions.thread_id,
      }),
    },
    bot_tokens: {
    ...
    thread_worktrees: {
      thread: r.one.thread_sessions({
        from: r.thread_worktrees.thread_id,
        to: r.thread_sessions.thread_id,
      }),
    },
    channel_directories: {
      channel_model: r.one.channel_models({
        from: r.channel_directories.channel_id,
        to: r.channel_models.channel_id,
      }),
      channel_agent: r.one.channel_agents({
        from: r.channel_directories.channel_id,
        to: r.channel_agents.channel_id,
      }),
      channel_worktree: r.one.channel_worktrees({
        from: r.channel_directories.channel_id,
        to: r.channel_worktrees.channel_id,
      }),
      channel_verbosity: r.one.channel_verbosity({
        from: r.channel_directories.channel_id,
        to: r.channel_verbosity.channel_id,
      }),
      channel_mention_mode: r.one.channel_mention_mode({
        from: r.channel_directories.channel_id,
        to: r.channel_mention_mode.channel_id,
      }),
      scheduled_tasks: r.many.scheduled_tasks(),
    },
    channel_models: {
      channel: r.one.channel_directories({
        from: r.channel_models.channel_id,
        to: r.channel_directories.channel_id,
      }),
    },
    session_models: {},
    channel_agents: {
      channel: r.one.channel_directories({
        from: r.channel_agents.channel_id,
        to: r.channel_directories.channel_id,
      }),
    },
    session_agents: {},
    channel_worktrees: {
      channel: r.one.channel_directories({
        from: r.channel_worktrees.channel_id,
        to: r.channel_directories.channel_id,
      }),
    },
    channel_verbosity: {
      channel: r.one.channel_directories({
        from: r.channel_verbosity.channel_id,
        to: r.channel_directories.channel_id,
      }),
    },
    channel_mention_mode: {
      channel: r.one.channel_directories({
        from: r.channel_mention_mode.channel_id,
        to: r.channel_directories.channel_id,
      }),
    },
    global_models: {
    ...
    scheduled_tasks: {
      channel: r.one.channel_directories({
        from: r.scheduled_tasks.channel_id,
        to: r.channel_directories.channel_id,
      }),
      thread: r.one.thread_sessions({
        from: r.scheduled_tasks.thread_id,
        to: r.thread_sessions.thread_id,
      }),
      session_start_sources: r.many.session_start_sources(),
    },
    session_start_sources: {
      scheduled_task: r.one.scheduled_tasks({
        from: r.session_start_sources.scheduled_task_id,
        to: r.scheduled_tasks.id,
      }),
    },
    forum_sync_configs: {
