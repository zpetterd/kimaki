...
export async function deleteScheduledTask(taskId: number) {
  const db = await getDb()
  await db.delete(schema.scheduled_tasks).where(orm.eq(schema.scheduled_tasks.id, taskId))
}
...
export async function setSessionStartSource({
  ...
  const db = await getDb()
  await db
    .insert(schema.session_start_sources)
    .values({
...
export async function getSessionStartSourcesBySessionIds(sessionIds: string[]) {
  ...
  for (let index = 0; index < sessionIds.length; index += chunkSize) {
    rows.push(
      ...(await db.query.session_start_sources.findMany({
...
export async function setChannelAgent(channelId: string, agentName: string) {
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: schema.channel_agents.channel_id,
      set: { agent_name: agentName, updated_at: new Date() },
    })
}
...
export async function setSessionAgent(sessionId: string, agentName: string) {
  const db = await getDb()
  await db
    .insert(schema.session_agents)
    .values({ session_id: sessionId, agent_name: agentName })
    .onConflictDoUpdate({
      target: schema.session_agents.session_id,
...
export async function getThreadWorktree(threadId: string) {
  const db = await getDb()
  return (
    (await db.query.thread_worktrees.findFirst({ where: { thread_id: threadId } })) ?? undefined
  )
}
...
export async function createPendingWorktree({
  ...
  await db.batch([
    ...
    db
      .insert(schema.thread_worktrees)
      .values({
        thread_id: threadId,
...
export async function setWorktreeReady({
  ...
  const db = await getDb()
  await db
    .update(schema.thread_worktrees)
    .set({ worktree_directory: worktreeDirectory, status: 'ready' })
...
export async function setChannelVerbosity(channelId: string, verbosity: VerbosityLevel) {
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: schema.channel_verbosity.channel_id,
      set: { verbosity, updated_at: new Date() },
    })
}
...
export async function setChannelMentionMode(channelId: string, enabled: boolean) {
  const db = await getDb()
  await db
    .insert(schema.channel_mention_mode)
    .values({ channel_id: channelId, enabled: enabled ? 1 : 0 })
    .onConflictDoUpdate({
      target: schema.channel_mention_mode.channel_id,
      set: { enabled: enabled ? 1 : 0, updated_at: new Date() },
    })
}
...
export async function getChannelWorktreesEnabled(channelId: string) {
  const db = await getDb()
  return (
    (await db.query.channel_worktrees.findFirst({ where: { channel_id: channelId } }))?.enabled ===
    1
  )
}
...
export async function setChannelWorktreesEnabled(channelId: string, enabled: boolean) {
  ...
  await db
    ...
    .values({ channel_id: channelId, enabled: enabled ? 1 : 0 })
    .onConflictDoUpdate({
      target: schema.channel_worktrees.channel_id,
      set: { enabled: enabled ? 1 : 0, updated_at: new Date() },
    })
}
...
export async function upsertThreadSession({
  ...
  await db
    ...
    .onConflictDoUpdate({
      ...
      set: { session_id: sessionId, source },
    })
}
...
export async function getCleanupPromptedAt(threadId: string) {
  const db = await getDb()
  const row = await db.query.thread_sessions.findFirst({
    where: { thread_id: threadId },
    columns: { cleanup_prompted_at: true },
  })
  return row?.cleanup_prompted_at ?? null
}

export async function setCleanupPromptedAt(threadId: string, date: Date) {
  const db = await getDb()
  await db
    .update(schema.thread_sessions)
    .set({ cleanup_prompted_at: date })
    .where(orm.eq(schema.thread_sessions.thread_id, threadId))
}
...
export async function appendSessionEventsSinceLastTimestamp({
  ...
  sessionId: string
  events: Array<typeof schema.session_events.$inferInsert>
}) {
  ...
  if (staleRows.length > 0) {
    await db.delete(schema.session_events).where(
      orm.inArray(
        schema.session_events.id,
...
export async function getPartMessageIds(threadId: string) {
  const db = await getDb()
  const rows = await db.query.part_messages.findMany({
    where: { thread_id: threadId },
...
export async function setPartMessage({
  ...
  const db = await getDb()
  await db
    .insert(schema.part_messages)
    .values({ part_id: partId, message_id: messageId, thread_id: threadId })
    .onConflictDoUpdate({
      target: schema.part_messages.part_id,
      set: { message_id: messageId, thread_id: threadId },
    })
}
...
function splitServiceAuthToken({
  token,
}: {
  token: string
...
> {
  const db = await getDb()
  const [row] = await db.query.bot_tokens.findMany({
...
export async function getGeminiApiKey(appId: string) {
  const db = await getDb()
  return (
    (await db.query.bot_api_keys.findFirst({ where: { app_id: appId } }))?.gemini_api_key ?? null
  )
}
...
export async function getOpenAIApiKey(appId: string) {
  const db = await getDb()
  return (
    (await db.query.bot_api_keys.findFirst({ where: { app_id: appId } }))?.openai_api_key ?? null
  )
}
 ...
 */
export async function getAnyAudioApiKey(): Promise<{
...
export async function getAllTextChannelDirectories() {
  const db = await getDb()
  const rows = await db.query.channel_directories.findMany({
...
export async function deleteChannelDirectoriesByDirectory(directory: string) {
  const db = await getDb()
  await db
    .delete(schema.channel_directories)
    .where(orm.eq(schema.channel_directories.directory, directory))
}
...
export async function deleteChannelDirectoryById(channelId: string) {
  ...
  await db.batch([
    ...
    db
      .delete(schema.channel_worktrees)
      .where(orm.eq(schema.channel_worktrees.channel_id, channelId)),
    db
      .delete(schema.channel_verbosity)
      .where(orm.eq(schema.channel_verbosity.channel_id, channelId)),
    db
      .delete(schema.channel_mention_mode)
      .where(orm.eq(schema.channel_mention_mode.channel_id, channelId)),
  ] as const)
  const rows = await db
    .delete(schema.channel_directories)
...
export async function getVoiceChannelDirectory(channelId: string) {
  const db = await getDb()
  return (
    await db.query.channel_directories.findFirst({
...
export async function findTextChannelByVoiceChannel(voiceChannelId: string) {
  const db = await getDb()
  const voiceChannel = await db.query.channel_directories.findFirst({
...
export async function upsertForumSyncConfig({
  ...
  const db = await getDb()
  await db
    .insert(schema.forum_sync_configs)
    .values({ app_id: appId, forum_channel_id: forumChannelId, output_dir: outputDir, direction })
    .onConflictDoUpdate({
      target: [schema.forum_sync_configs.app_id, schema.forum_sync_configs.forum_channel_id],
      set: { output_dir: outputDir, direction, updated_at: new Date() },
    })
}
...
export async function deleteForumSyncConfig({
  ...
  const db = await getDb()
  await db
    .delete(schema.forum_sync_configs)
    .where(
      orm.and(
        orm.eq(schema.forum_sync_configs.app_id, appId),
        orm.eq(schema.forum_sync_configs.forum_channel_id, forumChannelId),
      ),
    )
}
...
export async function createIpcRequest({
  ...
  const db = await getDb()
  const [row] = await db
    .insert(schema.ipc_requests)
    .values({ type, session_id: sessionId, thread_id: threadId, payload })
    .returning()
...
export async function claimPendingIpcRequests() {
  const db = await getDb()
  const pending = await db.query.ipc_requests.findMany({
  ...
  for (const req of pending) {
    const rows = await db
      .update(schema.ipc_requests)
      .set({ status: 'processing' })
      .where(
        orm.and(
          orm.eq(schema.ipc_requests.id, req.id),
...
export async function cancelStaleProcessingRequests({ ttlMs }: { ttlMs: number }) {
  ...
  const rows = await db
    ...
    .where(
      orm.and(
        orm.eq(schema.ipc_requests.status, 'processing'),
        orm.lt(schema.ipc_requests.updated_at, cutoff),
      ),
    )
